...
需在queue的最顶层设置，即root.acl_submit_applications和root.acl_administer_queue。如果不设置root.acl_submit_applications和root.acl_administer_queue时，默认所有子queue都会有所有权限（可将此两个配置的值都设置为空格，取消所有用户的权限)
<property>
...
参考：http://blog.csdn.net/opensure/article/details/78296877
#查看用户acl权限
